...
Invoke `update` when the user invokes `/update` or asks to move kingshand itself to the latest
version. It fast-forwards this installation to the latest tagged release, re-runs `install.ps1`,
and says which version it moved from and to. It refuses rather than proceeding on a dirty tree, on
any live worker, off the release branch, or where no release has been tagged yet. Never run it on
the King's behalf.
...
- `witness` - load before writing a `## Browser checks` section into a brief, and hand the worker
  the file itself under `Read first` rather than naming the skill: skills exist in this repository
  only, so a worker in another repo's worktree cannot load one. A brief with no such section gets
...
- `rally` - load when a worker reads dead or has no live process, or is reported stalled, found with
  ...
  worker's worktree before loading it.
