...
- A module under `bin\` imports a nested module **without** `-Force`; a script may force its own
  ...
  `Test-CrewPrereqs` printed every check OK and then lost `Get-KingshandHome`. Each module's import
  line carries a one-line reminder, and `tests\Index.Tests.ps1` and `tests\Projects.Tests.ps1` pin
  the two edges that regressed.
